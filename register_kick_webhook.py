...
async def register_webhook(broadcaster_user_id: str, webhook_url: str, access_token: str = None):
    ...
    try:
        ...
        for sub in existing:
            sub_dict = sub.__dict__ if hasattr(sub, "__dict__") else sub
            if (
                sub_dict.get("event") == "chat.message.sent"
                and sub_dict.get("broadcaster_user_id") == broadcaster_user_id
                and sub_dict.get("callback_url") == webhook_url
            ):
        ...
        subscription = await api.subscribe_webhook(
            event="chat.message.sent", callback_url=webhook_url, broadcaster_user_id=broadcaster_user_id
        )
...
Examples:
  ...
  python register_kick_webhook.py --broadcaster-id 152837 --webhook-url https://bot.lelebot.xyz/webhooks/kick --token YOUR_TOKEN
    ...
    parser.add_argument(
        "--token", type=str, help="OAuth access token (optional, will use client credentials if not provided)"
    )
    ...
    if args.list:
        ...
        asyncio.run(
            register_webhook(
                broadcaster_user_id=args.broadcaster_id, webhook_url=args.webhook_url, access_token=args.token
            )
        )
    else:
        parser.print_help()
