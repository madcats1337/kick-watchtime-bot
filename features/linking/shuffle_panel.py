...
async def verify_and_grant(interaction: discord.Interaction, engine, settings_getter, entered_username: str):
    ...
    # 1. Already-verified check (UNIQUE on discord_id)
    try:
        with engine.connect() as conn:
            existing = conn.execute(
                text("SELECT shuffle_username FROM raffle_shuffle_links WHERE discord_id = :d"),
                {"d": discord_id},
            ).fetchone()
        if existing:
...
def _insert_verified_link(engine, shuffle_username, kick_name, discord_id):
    ...
    try:
        with engine.begin() as conn:
            existing = conn.execute(
                text("SELECT discord_id FROM raffle_shuffle_links WHERE shuffle_username = :u"),
                {"u": shuffle_username},
            ).fetchone()
            if existing:
            ...
            conn.execute(
                text(
                    ...
                    VALUES
                        (:shuffle_username, :kick_name, :discord_id, TRUE, :verified_by, CURRENT_TIMESTAMP)
                    """
                ),
                {
